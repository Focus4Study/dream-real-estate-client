...
const ProfilePage = () => {
    ...
    return (
        <div className="bg-[url('https://i.ibb.co/0D967jZ/2151041355.jpg')] h-screen bg-center bg-cover bg-no-repeat pt-40 text-white">
            <Helmet>
                <title>DR-Estate | Profile</title>
            </Helmet>
            <div>
                <div className="flex flex-col bg-black bg-opacity-25 justify-center items-center mx-auto my-auto max-w-xs p-6 shadow-md rounded-xl sm:px-12 dark:bg-gray-50 dark:text-gray-800">
                    <img src={profile.photoURL} alt="" className="w-32 h-32 mx-auto rounded-full dark:bg-gray-500 aspect-square" />
                    <div className="space-y-4 text-center divide-y dark:divide-gray-300">
                        <div className="my-2 space-y-1">
                            <h2 className="text-3xl font-bold sm:text-2xl">{profile.name}</h2>
                            <p className="px-5 text-xl sm:text-base dark:text-gray-600">Access: {profile.role ? <span className="uppercase">{profile.role}</span> : 'User'}</p>
                            <p className="px-5 text-xl sm:text-base dark:text-gray-600">Email: {user.email}</p>
                            <p className="px-5 text-xl sm:text-base dark:text-gray-600">Joined: {user.metadata.creationTime.slice(0, 17)}</p>
                        </div>
